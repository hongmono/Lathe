...
현재 Space에 보이는 앱들이 먼저 나오고, 가운데 카드가 항상 포커스입니다 —
...
다른 인증서(본인 Developer ID, 팀 인증서 등)를 쓰고 싶다면 로컬 빌드 설정을
override 하면 됩니다:
...
| ⌘ 떼기                | 가운데(포커스) 앱·창 활성화                       |
| ⌘ 유지 + Esc 또는 .   | 활성화 없이 닫기                                  |
| ⌘ 유지 + H            | 전환 없이 포커스 앱 숨기기·다시 보이기            |
| ⌘ 유지 + Q            | 전환 없이 포커스 앱에 정상 종료 요청              |
| ⌘+`                   | 포커스 앱의 창 사이를 순환                        |
| ⌘ 유지 + ⇧`           | 창을 역방향으로 순환                              |
...
가져온 Developer ID Application identity는 실행 중에 자동으로 찾기 때문에,
인증서 subject와 Apple Team ID를 레포에 커밋하지 않아도 됩니다.
...
1. login keychain 에서 Developer ID Application 인증서 + private key 를
   ...
   # 이어 뜨는 다이얼로그에서
   # 본인의 Developer ID Application 인증서를 선택합니다.
   ```
...
4. Sparkle EdDSA key를 생성하고 public/private key를 모두 GitHub secret으로
   복사합니다. public key는 민감값은 아니지만, 서명 설정을 레포에 직접
   남기지 않기 위해 secret으로 관리합니다:

   ```bash
   generate_keys
   # 생성된 public key를 SPARKLE_PUBLIC_ED_KEY로 복사합니다.
...
5. GitHub 에서 **Settings → Secrets and variables → Actions → New
   ...
   | `SPARKLE_PUBLIC_ED_KEY`       | 4단계에서 생성한 Sparkle EdDSA public key           |
