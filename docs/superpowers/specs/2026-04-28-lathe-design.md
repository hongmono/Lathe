...
Lathe는 macOS의 ⌘+Tab을 가로채 시스템 스위처 대신 **부채꼴 캐러셀 오버레이**를 띄운다. 화면 중앙(마우스가 있는 스크린 기준)에 카드를 부채꼴로 펼치고, **가운데 카드 = 현재 포커스**라는 고정된 시각 모델을 쓴다. ⌘ 누른 채 Tab으로 카드를 회전시키고, ⌘ 떼면 가운데 앱이 활성화된다.

코드 베이스를 작게 유지해 본인이 직접 audit·서명·빌드할 수 있도록 하는 것이 본 프로젝트의 또 다른 목표다.
