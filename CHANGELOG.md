...
- 창·브라우저 탭 목록이 기본적으로 0.3초 뒤에 나타나며, 시작 지연을 0초부터 1초까지 조절할 수 있습니다.
- 카드와 창 목록의 애니메이션 설정을 별도의 '애니메이션' 화면으로 모아 스타일, 속도, 시작 지연과 미리보기를 한곳에서 관리할 수 있습니다.
- 같은 앱의 여러 창을 넘길 때 선택된 창 제목이 잠시 사라졌다 다시 나타나던 현상을 고쳤습니다.

## 1.9.2

- '아래로 펼치기'에서 첫 번째 창은 고정하고, 나머지 창은 서로 겹치지 않은 채 위에서 아래로 차례대로 내려오도록 움직임을 더 분명하게 다듬었습니다.

## 1.9.1

- '아래로 펼치기' 애니메이션에서 아래쪽 창 제목이 위쪽 항목과 겹쳐 보이지 않도록, 창들을 뒤에 고정한 채 배경 상자가 열리면서 드러나도록 다듬었습니다.

## 1.9.0

- 설정 → 캐러셀에서 여러 창과 브라우저 탭 목록의 등장 방식을 '전체 등장', '순차 등장', '아래로 펼치기' 중에서 선택할 수 있습니다.
- '아래로 펼치기'는 첫 번째 창을 고정한 채 나머지 창과 배경 상자가 아래로 자연스럽게 확장됩니다.
- 설정 화면에서 선택한 효과를 바로 미리 보거나 다시 재생할 수 있고, 애니메이션 속도를 0.5배부터 2배까지 조절할 수 있습니다.

## 1.8.2

- 여러 창과 브라우저 탭 목록이 나타날 때 보이는 항목이 위에서 아래로 차례대로 내려오도록 캐러셀의 펼침 동작과 통일했습니다. 빠른 앱 전환은 지연하지 않으며, macOS의 '동작 줄이기'가 켜져 있으면 이동 없이 페이드로 표시합니다.

## 1.8.1

- 캐러셀이 열릴 때 여러 창과 브라우저 탭 목록도 카드와 함께 자연스럽게 나타납니다.
- 창·탭 선택 표시와 긴 목록의 스크롤 이동을 부드럽게 다듬고, macOS의 '동작 줄이기'가 켜져 있으면 이동 대신 짧은 페이드를 사용합니다.

## 1.8.0

- 설정 → 캐러셀에서 '⌘`으로 브라우저 탭 표시'를 켜면 지원되는 브라우저의 탭을 창별로 확인하고 `⌘+백틱`으로 순환할 수 있습니다.
- 브라우저 탭은 기존 손쉬운 사용 권한으로 읽고 선택하며, 탭을 사용할 수 없는 브라우저에서는 기존 창 전환을 그대로 사용합니다.

## 1.7.0

- 카카오톡처럼 모든 창을 닫아도 계속 실행되는 앱을 `⌘+Tab`으로 선택하면 기본 창을 다시 엽니다.
- 설정 → 일반의 '창이 없는 앱 다시 열기' 옵션으로 재열기 동작을 끄거나 다시 켤 수 있습니다.

## 1.6.0

- `⌘+Tab` 전환 화면에서 `⌘H`를 누르면 선택한 앱을 전환 없이 숨기거나 다시 표시할 수 있습니다. 숨긴 앱은 캐러셀에서 흐리게 표시되고 상태 아이콘이 붙습니다.
- 전환 화면에서 `⌘Q`를 누르면 선택한 앱에 정상 종료를 요청합니다. 저장 확인은 그대로 유지하고 Finder는 종료하지 않으며, 종료한 앱이 빠지면 다음 앱으로 선택이 자연스럽게 이어집니다.
- `⌘.`를 눌러도 `Esc`와 마찬가지로 현재 선택을 실행하지 않고 전환 화면을 닫을 수 있습니다.
- 배포 전에 전체 단위 테스트를 먼저 통과하도록 릴리스 파이프라인을 강화했습니다.

## 1.5.0

- `⌘+Tab` 캐러셀이 열릴 때 카드가 중심에서 최종 레이아웃으로 자연스럽게 펼쳐집니다. 설정 → 캐러셀의 '열릴 때 카드 펼치기'로 효과를 끄거나 다시 켤 수 있습니다.
- macOS의 '동작 줄이기'가 켜져 있으면 카드 펼침 효과를 자동으로 생략하며, 효과를 끄더라도 앱 전환 중의 기존 카드 이동 애니메이션은 그대로 유지됩니다.

## 1.4.7

- `⌘+Tab` 캐러셀이 열릴 때 카드가 이전 선택 위치에서 불필요하게 움직이던 현상을 고쳤습니다. 첫 표시에서는 카드가 최종 위치에 바로 배치되고, 이후 전환 애니메이션은 그대로 유지됩니다.

## 1.4.6

- `⌘+Tab` 캐러셀 끝단에서 카드가 사라지거나 나타날 때 패널 경계에 잘려 보이던 전환을, 사용 가능한 여백 안의 짧은 방향 이동과 페이드로 다듬었습니다.

## 1.4.5

- `⌘+Tab` 전환 화면이 열린 동안 `⌘,`를 누르면 현재 선택을 실행하지 않고 전환 화면을 닫은 뒤 Lathe 일반 설정을 바로 엽니다. 메뉴바 아이콘을 찾기 어려운 상태에서도 설정창에 진입할 수 있습니다.

## 1.4.4

- `⌘+Tab` 캐러셀에서 마지막 앱 다음에 첫 앱으로 넘어갈 때 카드가 반대편으로 튀던 움직임을 자연스러운 순환 전환으로 다듬었습니다. 끝단 카드는 이동 방향에 맞춰 퇴장·진입하고, 실행 중 앱 목록이 갱신되어도 전환 위치가 흔들리지 않습니다.

## 1.4.3

- macOS가 Lathe 메뉴바 아이콘을 숨김 상태로 기억해 앱이 실행 직후 종료되던 문제를 고쳤습니다. 이제 Lathe를 실행하면 메뉴바 아이콘이 항상 다시 나타납니다.

## 1.4.2

- 같은 앱의 여러 창을 `⌘+\``로 전환할 때 선택 글자가 즉시 또렷하게 바뀌도록 다듬고, 반복 입력 중 불필요한 화면 갱신을 줄여 더 빠르고 자연스럽게 동작합니다.

## 1.4.1

- 미션 컨트롤이 열릴 때의 버벅임을 줄였습니다. 창 미리보기를 실제 표시 크기에 맞춰 캡처하고, 미리보기가 채워질 때 화면을 한 번에 갱신해 창이 많아도 더 매끄럽게 펼쳐집니다.

## 1.4.0

- 미션 컨트롤·캐러셀에서 마우스로 카드를 클릭해 바로 그 창(앱)으로 전환할 수 있습니다. ⌘ 을 누른 채 원하는 카드를 클릭하면 됩니다.
- 마우스를 올린 카드는 어둠이 걷혀 밝아지고 커서가 손 모양으로 바뀌어, 클릭할 대상을 한눈에 알 수 있습니다. 강조 테두리(선택 표시)는 그대로 ⌘+Tab(키보드)으로만 움직입니다.

## 1.3.2

- 미션 컨트롤에서 ⌘+Tab 이동 순서를 화면(모니터) 순으로 정리했습니다. 한 모니터의 앱들을 모두 돈 뒤 다음 모니터로 넘어가, 순서가 화면을 넘나들지 않습니다.
- 미션 컨트롤이 뜨는 속도를 크게 높였습니다. 창 목록을 한 번에 읽어와 앱·창이 많아도 지연이 거의 늘지 않습니다.
- 세로로 돌린 모니터에서 창 하나가 화면을 꽉 채울 만큼 과하게 확대되던 문제를 고쳤습니다. 이제 실제 창 크기에 맞춰(확대 없이) 표시합니다.
- 일부 앱에서 같은 창이 스택에 두 장 겹쳐 보이던 문제를 고쳤습니다. (그림자·래퍼 창을 따로 만드는 앱)
- 썸네일을 병렬로 캡처하고 나타나는 애니메이션을 짧게 줄여, 앱 아이콘에서 미리보기로 더 빠르게 바뀝니다.

## 1.3.1

- 미션 컨트롤에서 같은 앱의 창들을 하나의 스택으로 묶어 보여줍니다. ⌘+Tab 으로 앱(스택) 사이를 이동하고, ⌘+백틱(⇧ 와 함께 누르면 역방향)으로 선택한 앱의 창을 하나씩 넘길 수 있습니다.
- 창을 넘기면 카드가 실제로 자리를 옮기며 부드럽게 이동합니다. 스택은 뒤 카드가 살짝 삐져나와 창이 여러 개임을 보여줍니다.

## 1.3.0

- 미션 컨트롤 레이아웃을 추가했습니다. 설정 → 레이아웃에서 '미션 컨트롤'을 고르면, ⌘+Tab 이 현재 Space 의 모든 창을 각자 속한 모니터 위에 겹치지 않게 펼쳐 보여줍니다. ⌘ 을 누른 채 Tab 을 눌러 창을 하나씩 넘기고, 선택한 창만 선명하게·나머지는 살짝 어둡게 표시하며, ⌘ 을 떼면 그 창으로 전환합니다.
- 각 창은 실제 창 미리보기(썸네일)로 표시되고, 뜰 때 부드럽게 나타납니다. 미리보기에는 화면 기록 권한이 필요하며, 권한이 없으면 앱 아이콘과 제목으로 대신 표시합니다.
